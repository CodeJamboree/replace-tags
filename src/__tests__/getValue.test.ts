...
describe("getValue", () => {
  ...
  describe('functional values', () => {
    ...
    it('passes key as arg1 to function', () => {
      ...
      expect(nameCallback).toHaveBeenLastCalledWith(key, expect.anything(), expect.anything());
    });
    it('passes currentPath as arg2 to function', () => {
      ...
      const fullPath = 'fullPath';
      getValue(source, key, currentPath, fullPath);
      expect(nameCallback).toHaveBeenLastCalledWith(expect.anything(), currentPath, expect.anything());
    });
    it('passes fullPath as arg3 to function', () => {
      ...
      const fullPath = 'fullPath';
      getValue(source, key, currentPath, fullPath);
      expect(nameCallback).toHaveBeenLastCalledWith(expect.anything(), expect.anything(), fullPath);
    });
  });
})
