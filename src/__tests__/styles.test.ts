...
describe("styles", () => {
    ...
    it('replaces {{{triple curley bracess}}}', () => {
      ...
      expect(replaceTags(text, values, TripleCurlyBraces)).toBe('Hello John Doe!')
    })
    ...
    it('replaces [:square brackets with colons:]', () => {
      const text = "Hello [:name:]!";
      expect(replaceTags(text, values, SquareBracketsWithColons)).toBe('Hello John Doe!')
    })
    ...
    it('replaces ${Dollar Sign with Curly Braces}', () => {
      const text = "Hello ${name}!";
      expect(replaceTags(text, values, DollarSignWithCurlyBraces)).toBe('Hello John Doe!')
    })
    ...
    it('replaces {$Curly Braces with Dollar Signs$}', () => {
      const text = "Hello {$name$}!";
      expect(replaceTags(text, values, CurlyBracesWithDollarSigns)).toBe('Hello John Doe!')
    })
    it('replaces [[Double Square Brackets]]', () => {
      const text = "Hello [[name]]!";
      expect(replaceTags(text, values, DoubleSquareBrackets)).toBe('Hello John Doe!')
    })
    it('replaces <% Angle Brackets with Percent Signs %>', () => {
      const text = "Hello <%name%>!";
      expect(replaceTags(text, values, AngleBracketsWithPercentSigns)).toBe('Hello John Doe!')
    })
    it('replaces [- Square Brackets with Dashes -]', () => {
      const text = "Hello [-name-]!";
      expect(replaceTags(text, values, SquareBracketsWithDashes)).toBe('Hello John Doe!')
    })
    it('replaces __ Double Underscores __', () => {
      const text = "Hello __name__!";
      expect(replaceTags(text, values, DoubleUnderscores)).toBe('Hello John Doe!')
    })
    ...
    it('replaces {{% Double Curly Braces with Percent Sign %}}', () => {
      const text = "Hello {{%name%}}!";
      expect(replaceTags(text, values, DoubleCurlyBracesWithPercentSign)).toBe('Hello John Doe!')
    })
    it('replaces ::{ Double Colons with Braces }::', () => {
      const text = "Hello ::{name}::!";
      expect(replaceTags(text, values, DoubleColonsWithBraces)).toBe('Hello John Doe!')
    })
    it('replaces [[$ Double Square Brackets with Dollar Signs $]]', () => {
      const text = "Hello [[$name$]]!";
      expect(replaceTags(text, values, DoubleSquareBracketsWithDollarSigns)).toBe('Hello John Doe!')
    })
    it('replaces {# Curly Braces with Pound Signs #}', () => {
      const text = "Hello {#name#}!";
      expect(replaceTags(text, values, CurlyBracesWithPoundSigns)).toBe('Hello John Doe!')
    })
    it('replaces ^^{ Double Carets with Braces }^^', () => {
      const text = "Hello ^^{name}^^!";
      expect(replaceTags(text, values, DoubleCaretsWithBraces)).toBe('Hello John Doe!')
    })
    it('replaces {! Single Braces with Exclamation Mark !}', () => {
      const text = "Hello {!name!}!";
      expect(replaceTags(text, values, SingleCurlyBracesWithExclamationMarks)).toBe('Hello John Doe!')
    })
    it('replaces ?? Double Question Marks ??', () => {
      const text = "Hello ??name??!";
      expect(replaceTags(text, values, DoubleQuestionMarks)).toBe('Hello John Doe!')
    })
    it('replaces #{ Hash Symbols with Curly Braces }#', () => {
      const text = "Hello #{name}#!";
      expect(replaceTags(text, values, HashSymbolsWithCurlyBraces)).toBe('Hello John Doe!')
    })
